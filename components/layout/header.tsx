            ...
            {navLinks.map(link => (
              link.label === 'Events' ? (
                <div key={link.href} className="relative group">
                  <Link href={link.href} className="text-gray-700 hover:text-[#34c5c5] transition-colors">Events</Link>
                  <div className="absolute left-0 top-full mt-2 hidden group-hover:block bg-white border border-gray-200 rounded-lg shadow-lg min-w-[220px] p-2">
                    {[
                      { href: '/bombshell-bootcamp', label: 'Bombshell' },
                      { href: '/retreat', label: 'Retreats' },
                      { href: '/vision-board', label: 'Vision Board' },
                      { href: '/masterclass', label: 'Masterclass' },
                      { href: '/speaker', label: 'Speaking' },
                    ].map((item) => (
                      <Link key={item.href} href={item.href} className="block px-3 py-2 rounded-md text-sm text-gray-700 hover:bg-teal-50 hover:text-[#34c5c5]">{item.label}</Link>
                    ))}
                  </div>
                </div>
              ) : (
