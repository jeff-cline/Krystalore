'use client'

import React, { useEffect, useRef, useCallback } from 'react'

/**
 * KrystaloreDiamondV2 — Fixed version with:
 * 1. Correct face culling (text faces TOWARD viewer)
 * 2. Complete crown geometry (no gaps)
 * 3. Proper 3D brilliant-cut diamond shape
 */

interface Vec3 { x: number; y: number; z: number }
interface Face {
  verts: Vec3[]
  label: string
  link: string
  color: string
  type: 'crown' | 'pavilion' | 'table'
}

const TEAL = { r: 45, g: 212, b: 191 }

const CROWN_LABELS = ['Core', 'Confidence', 'Consistency', 'Community', 'Celebration']
const CROWN_LINKS = ['#core', '#confidence', '#consistency', '/community', '#celebration']

const PAV_LABELS = [
  'Fitness', 'Speaking', 'Corp. Retreats', 'Private', 'Books',
  'Podcast', 'Courses', 'Veterans', 'Real Estate', 'Gypsy Tours',
  'Business', 'Retreats', 'Nonprofits', 'Partners', 'Boot Camp',
]
const PAV_LINKS = [
  '/fitness', '/keynote-speaker', '/corporate-retreat-planning', '/private', '/books',
  '/podcasts', '/courses', '/veteran-coaching', '/partners', '/gypsy-tours',
  '/bootcamp', '/retreat', '/community', '/partners', '/bootcamp',
]

function rotateY(v: Vec3, angle: number): Vec3 {
  const c = Math.cos(angle), s = Math.sin(angle)
  return { x: v.x * c + v.z * s, y: v.y, z: -v.x * s + v.z * c }
}
function rotateX(v: Vec3, angle: number): Vec3 {
  const c = Math.cos(angle), s = Math.sin(angle)
  return { x: v.x, y: v.y * c - v.z * s, z: v.y * s + v.z * c }
}

function project(v: Vec3, cx: number, cy: number, fov: number): { x: number; y: number; scale: number } {
  const z = v.z + fov
  const scale = fov / Math.max(z, 1)
  return { x: cx + v.x * scale, y: cy + v.y * scale, scale }
}

function faceCenter(verts: Vec3[]): Vec3 {
  const n = verts.length
  let x = 0, y = 0, z = 0
  for (const v of verts) { x += v.x; y += v.y; z += v.z }
  return { x: x / n, y: y / n, z: z / n }
}

// Cross product for normal, normalized
function faceNormalNorm(verts: Vec3[]): Vec3 {
  const a = verts[0], b = verts[1], c = verts[2]
  const ux = b.x - a.x, uy = b.y - a.y, uz = b.z - a.z
  const vx = c.x - a.x, vy = c.y - a.y, vz = c.z - a.z
  const nx = uy * vz - uz * vy
  const ny = uz * vx - ux * vz
  const nz = ux * vy - uy * vx
  const len = Math.sqrt(nx * nx + ny * ny + nz * nz) || 1
  return { x: nx / len, y: ny / len, z: nz / len }
}

interface Sparkle {
  angle: number
  yPos: number
  phase: number
  speed: number
  size: number
}

interface DiamondProps {
  size?: number
  className?: string
  speed?: number
}

export default function KrystaloreDiamondV2({ size = 500, className = '', speed = 12 }: DiamondProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const frameRef = useRef<number>(0)
  const sparklesRef = useRef<Sparkle[]>([])
  const facesRef = useRef<Face[]>([])
  const clickMapRef = useRef<{ path: Path2D; link: string }[]>([])

  const buildGeometry = useCallback(() => {
    const R = size * 0.30        // girdle radius
    const tableR = R * 0.52      // table radius
    const tableH = -size * 0.20  // table height (above center)
    const girdleH = 0            // girdle at Y=0
    const pavilionH = size * 0.35 // culet (bottom point)

    const faces: Face[] = []
    const girdleVerts: Vec3[] = []
    const tableVerts: Vec3[] = []

    // 15 girdle points
    for (let i = 0; i < 15; i++) {
      const a = (i / 15) * Math.PI * 2
      girdleVerts.push({ x: Math.cos(a) * R, y: girdleH, z: Math.sin(a) * R })
    }

    // 5 table points (offset by half-step so crown facets are centered)
    for (let i = 0; i < 5; i++) {
      const a = (i / 5) * Math.PI * 2 + Math.PI / 5
      tableVerts.push({ x: Math.cos(a) * tableR, y: tableH, z: Math.sin(a) * tableR })
    }

    // ===== CROWN FACETS =====
    // For each of the 5 crown sections, we need to fill the ENTIRE area
    // from the table edge down to the 3 girdle vertices with NO gaps.
    for (let i = 0; i < 5; i++) {
      const t1 = i
      const t2 = (i + 1) % 5
      const g0 = i * 3           // first girdle vert for this section
      const g1 = (i * 3 + 1) % 15
      const g2 = (i * 3 + 2) % 15
      const g3 = ((i + 1) * 3) % 15  // first girdle vert of next section

      // Main crown facet: table edge to outer girdle points
      // Reverse winding so normal points OUTWARD (toward viewer when front-facing)
      faces.push({
        verts: [girdleVerts[g0], girdleVerts[g3], tableVerts[t2], tableVerts[t1]],
        label: CROWN_LABELS[i],
        link: CROWN_LINKS[i],
        color: 'crown',
        type: 'crown',
      })

      // Fill star facet 1: triangle t1 → g0 → g1 (reversed winding)
      faces.push({
        verts: [girdleVerts[g1], girdleVerts[g0], tableVerts[t1]],
        label: '',
        link: CROWN_LINKS[i],
        color: 'star',
        type: 'crown',
      })

      // Fill star facet 2: triangle between g1 and g2 (middle gap filler)
      faces.push({
        verts: [girdleVerts[g2], girdleVerts[g1], tableVerts[t1]],
        label: '',
        link: CROWN_LINKS[i],
        color: 'star',
        type: 'crown',
      })

      // Fill star facet 3: triangle t2 → g2 → g3 (reversed winding)
      faces.push({
        verts: [girdleVerts[g3], girdleVerts[g2], tableVerts[t2]],
        label: '',
        link: CROWN_LINKS[(i + 1) % 5],
        color: 'star',
        type: 'crown',
      })
    }

    // Table face (pentagon) — reversed winding for outward normal (upward)
    faces.push({
      verts: [...tableVerts].reverse(),
      label: '',
      link: '#',
      color: 'table',
      type: 'table',
    })

    // ===== PAVILION FACETS =====
    const culet: Vec3 = { x: 0, y: pavilionH, z: 0 }
    for (let i = 0; i < 15; i++) {
      const g1 = girdleVerts[i]
      const g2 = girdleVerts[(i + 1) % 15]
      // Reversed winding: culet → g2 → g1 so normal points outward
      faces.push({
        verts: [culet, g1, g2],
        label: PAV_LABELS[i],
        link: PAV_LINKS[i],
        color: 'pavilion',
        type: 'pavilion',
      })
    }

    facesRef.current = faces

    // Sparkles
    const sparkles: Sparkle[] = []
    for (let i = 0; i < 25; i++) {
      sparkles.push({
        angle: Math.random() * Math.PI * 2,
        yPos: -0.3 + Math.random() * 0.9,
        phase: Math.random() * Math.PI * 2,
        speed: 0.8 + Math.random() * 2,
        size: 2 + Math.random() * 4,
      })
    }
    sparklesRef.current = sparkles
  }, [size])

  useEffect(() => {
    buildGeometry()
  }, [buildGeometry])

  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const ctx = canvas.getContext('2d')
    if (!ctx) return

    const dpr = typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1
    canvas.width = size * dpr
    canvas.height = size * dpr
    ctx.scale(dpr, dpr)

    const cx = size / 2
    const cy = size * 0.38
    const fov = size * 1.8

    let start: number | null = null

    const render = (ts: number) => {
      if (!start) start = ts
      const elapsed = (ts - start) / 1000
      const rotAngle = (elapsed / speed) * Math.PI * 2
      const tiltAngle = 0.18 // slight tilt toward viewer

      ctx.clearRect(0, 0, size, size)

      const transformed: {
        face: Face
        projected: { x: number; y: number; scale: number }[]
        avgZ: number
        normal: Vec3
      }[] = []

      for (const face of facesRef.current) {
        const rotated = face.verts.map(v => {
          let r = rotateY(v, rotAngle)
          r = rotateX(r, tiltAngle)
          return r
        })

        const normal = faceNormalNorm(rotated)

        // FRONT-FACE culling: only draw faces whose normal points TOWARD camera (negative Z)
        // The viewer is at z = -infinity, looking toward +Z
        // Face normal with negative Z = pointing toward viewer = FRONT face
        // We SKIP faces where normal.z >= 0 (pointing away = back face)
        if (normal.z >= 0 && face.type !== 'table') continue

        const proj = rotated.map(v => project(v, cx, cy, fov)) as { x: number; y: number; scale: number }[]
        const avgZ = rotated.reduce((s, v) => s + v.z, 0) / rotated.length

        transformed.push({ face, projected: proj, avgZ, normal })
      }

      // Painter's algorithm: draw far faces first
      transformed.sort((a, b) => b.avgZ - a.avgZ)

      const clickMap: { path: Path2D; link: string }[] = []

      for (const { face, projected, normal } of transformed) {
        const path = new Path2D()
        path.moveTo(projected[0].x, projected[0].y)
        for (let i = 1; i < projected.length; i++) {
          path.lineTo(projected[i].x, projected[i].y)
        }
        path.closePath()

        // Lighting
        const lightDir = { x: 0.3, y: -0.5, z: -0.8 }
        const dot = -(normal.x * lightDir.x + normal.y * lightDir.y + normal.z * lightDir.z)
        const brightness = 0.35 + Math.max(0, dot) * 0.65

        let r: number, g: number, b: number, alpha: number

        if (face.color === 'table') {
          r = Math.round(80 + brightness * 120)
          g = Math.round(230 + brightness * 25)
          b = Math.round(225 + brightness * 30)
          alpha = 0.75
        } else if (face.color === 'crown') {
          r = Math.round(TEAL.r * (0.6 + brightness * 0.5))
          g = Math.round(TEAL.g * (0.6 + brightness * 0.4))
          b = Math.round(TEAL.b * (0.6 + brightness * 0.5))
          alpha = 0.7 + brightness * 0.25
        } else if (face.color === 'star') {
          r = Math.round(TEAL.r * (0.5 + brightness * 0.45))
          g = Math.round(TEAL.g * (0.65 + brightness * 0.35))
          b = Math.round(TEAL.b * (0.6 + brightness * 0.45))
          alpha = 0.65 + brightness * 0.25
        } else {
          r = Math.round(TEAL.r * (0.4 + brightness * 0.5))
          g = Math.round(TEAL.g * (0.5 + brightness * 0.45))
          b = Math.round(TEAL.b * (0.5 + brightness * 0.5))
          alpha = 0.65 + brightness * 0.3
        }

        ctx.fillStyle = `rgba(${Math.min(255, r)}, ${Math.min(255, g)}, ${Math.min(255, b)}, ${alpha})`
        ctx.fill(path)

        // Specular highlight
        if (brightness > 0.7) {
          const specAlpha = (brightness - 0.7) * 1.5
          ctx.fillStyle = `rgba(255, 255, 255, ${specAlpha * 0.3})`
          ctx.fill(path)
        }

        // Edge lines
        ctx.strokeStyle = `rgba(180, 255, 250, ${0.15 + brightness * 0.35})`
        ctx.lineWidth = 0.8
        ctx.stroke(path)

        if (face.link !== '#') {
          clickMap.push({ path, link: face.link })
        }

        // ===== DRAW LABEL TEXT =====
        // Text follows the LEFT EDGE of the facet (the natural facet line)
        if (face.label) {
          let edgeTopX: number, edgeTopY: number
          let edgeBotX: number, edgeBotY: number

          if (face.type === 'crown') {
            // Crown quad (reversed winding): [girdle0, girdle3, table2, table1]
            // Left side edge: table1 (index 3) → girdle0 (index 0)
            edgeTopX = projected[3].x
            edgeTopY = projected[3].y
            edgeBotX = projected[0].x
            edgeBotY = projected[0].y
          } else {
            // Pavilion triangle (reversed): [culet, girdle1, girdle2]
            // Left side edge: girdle1 (index 1) → culet (index 0)
            edgeTopX = projected[1].x
            edgeTopY = projected[1].y
            edgeBotX = projected[0].x
            edgeBotY = projected[0].y
          }

          // Angle of the facet edge line
          const edgeAngle = Math.atan2(edgeBotY - edgeTopY, edgeBotX - edgeTopX)

          // Position along the edge
          const textFrac = face.type === 'crown' ? 0.5 : 0.35
          const edgePtX = edgeTopX + (edgeBotX - edgeTopX) * textFrac
          const edgePtY = edgeTopY + (edgeBotY - edgeTopY) * textFrac

          // Offset slightly into the face center
          let fcx = 0, fcy = 0
          for (const p of projected) { fcx += p.x; fcy += p.y }
          fcx /= projected.length
          fcy /= projected.length
          const insetFrac = face.type === 'crown' ? 0.35 : 0.3
          const textX = edgePtX + (fcx - edgePtX) * insetFrac
          const textY = edgePtY + (fcy - edgePtY) * insetFrac

          const avgScale = projected.reduce((s, p) => s + p.scale, 0) / projected.length
          const fontSize = face.type === 'crown'
            ? Math.max(9, Math.min(15, 13 * avgScale))
            : Math.max(6, Math.min(11, 9 * avgScale))

          const textAlpha = Math.max(0, Math.min(1, brightness * 1.5))

          ctx.save()
          ctx.translate(textX, textY)
          let drawAngle = edgeAngle
          if (drawAngle < -Math.PI / 2) drawAngle += Math.PI
          if (drawAngle > Math.PI / 2) drawAngle -= Math.PI
          ctx.rotate(drawAngle)

          ctx.font = `bold ${fontSize}px 'Inter', 'Segoe UI', sans-serif`
          ctx.textAlign = 'center'
          ctx.textBaseline = 'middle'

          ctx.fillStyle = `rgba(0, 0, 0, ${textAlpha * 0.9})`
          ctx.strokeStyle = `rgba(255, 255, 255, ${textAlpha * 0.4})`
          ctx.lineWidth = 2.5
          ctx.strokeText(face.label, 0, 0)
          ctx.fillText(face.label, 0, 0)
          ctx.restore()
        }
      }

      clickMapRef.current = clickMap

      // ===== SPARKLE EFFECTS =====
      for (const s of sparklesRef.current) {
        s.phase += s.speed * 0.016
        const osc = Math.sin(s.phase)
        if (osc > 0.3) {
          const sparkleAlpha = (osc - 0.3) / 0.7
          const sa = s.angle + rotAngle * 0.3
          const sr = size * 0.22 * (0.3 + Math.abs(s.yPos) * 0.5)
          let sv: Vec3 = {
            x: Math.cos(sa) * sr,
            y: s.yPos * size * 0.25,
            z: Math.sin(sa) * sr,
          }
          sv = rotateY(sv, rotAngle)
          sv = rotateX(sv, tiltAngle)

          if (sv.z < fov * 0.3) {
            const sp = project(sv, cx, cy, fov)
            ctx.save()
            ctx.globalAlpha = sparkleAlpha * 0.9
            const ss = s.size * sparkleAlpha
            ctx.strokeStyle = 'white'
            ctx.lineWidth = 1.5
            ctx.beginPath()
            ctx.moveTo(sp.x - ss, sp.y); ctx.lineTo(sp.x + ss, sp.y)
            ctx.moveTo(sp.x, sp.y - ss); ctx.lineTo(sp.x, sp.y + ss)
            ctx.moveTo(sp.x - ss * 0.5, sp.y - ss * 0.5); ctx.lineTo(sp.x + ss * 0.5, sp.y + ss * 0.5)
            ctx.moveTo(sp.x + ss * 0.5, sp.y - ss * 0.5); ctx.lineTo(sp.x - ss * 0.5, sp.y + ss * 0.5)
            ctx.stroke()

            const glow = ctx.createRadialGradient(sp.x, sp.y, 0, sp.x, sp.y, ss * 2)
            glow.addColorStop(0, `rgba(255, 255, 255, ${sparkleAlpha * 0.5})`)
            glow.addColorStop(1, 'rgba(255, 255, 255, 0)')
            ctx.fillStyle = glow
            ctx.beginPath()
            ctx.arc(sp.x, sp.y, ss * 2, 0, Math.PI * 2)
            ctx.fill()
            ctx.restore()
          }
        }
        if (s.phase > Math.PI * 4) {
          s.phase = 0
          s.angle = Math.random() * Math.PI * 2
          s.yPos = -0.3 + Math.random() * 0.9
        }
      }

      // Ambient glow
      const glowPulse = 0.06 + Math.sin(elapsed * 1.5) * 0.03
      const ambientGlow = ctx.createRadialGradient(cx, cy, size * 0.05, cx, cy, size * 0.45)
      ambientGlow.addColorStop(0, `rgba(45, 212, 191, ${glowPulse})`)
      ambientGlow.addColorStop(1, 'rgba(45, 212, 191, 0)')
      ctx.fillStyle = ambientGlow
      ctx.fillRect(0, 0, size, size)

      frameRef.current = requestAnimationFrame(render)
    }

    frameRef.current = requestAnimationFrame(render)
    return () => cancelAnimationFrame(frameRef.current)
  }, [size, speed])

  const handleClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current
    if (!canvas) return
    const rect = canvas.getBoundingClientRect()
    const dpr = window.devicePixelRatio || 1
    const x = (e.clientX - rect.left) * dpr
    const y = (e.clientY - rect.top) * dpr
    const ctx = canvas.getContext('2d')
    if (!ctx) return
    for (let i = clickMapRef.current.length - 1; i >= 0; i--) {
      if (ctx.isPointInPath(clickMapRef.current[i].path, x, y)) {
        const link = clickMapRef.current[i].link
        if (link.startsWith('#')) window.location.hash = link
        else window.location.href = link
        break
      }
    }
  }, [])

  const handleMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current
    if (!canvas) return
    const rect = canvas.getBoundingClientRect()
    const dpr = window.devicePixelRatio || 1
    const x = (e.clientX - rect.left) * dpr
    const y = (e.clientY - rect.top) * dpr
    const ctx = canvas.getContext('2d')
    if (!ctx) return
    let found = false
    for (let i = clickMapRef.current.length - 1; i >= 0; i--) {
      if (ctx.isPointInPath(clickMapRef.current[i].path, x, y)) { found = true; break }
    }
    canvas.style.cursor = found ? 'pointer' : 'default'
  }, [])

  return (
    <div className={`relative ${className}`} style={{ width: size, height: size }}>
      <canvas
        ref={canvasRef}
        style={{ width: size, height: size }}
        className="absolute inset-0"
        onClick={handleClick}
        onMouseMove={handleMove}
      />
    </div>
  )
}
