...
const DIM_META: Record<string, { icon: any; color: string }> = {
  Readiness: { icon: Target, color: '#e07800' },
  Resilience: { icon: ShieldCheck, color: '#0D9488' },
  Focus: { icon: Brain, color: '#6366f1' },
  Steadiness: { icon: Waves, color: '#34c5c5' },
  Drive: { icon: Zap, color: '#E8A849' },
  Recovery: { icon: HeartPulse, color: '#ec4899' },
}
...
export default function CheckPage() {
  ...
  const submitLead = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    const lead = { name, email, phone, help, scores: profile, source: 'check', ts: Date.now() }
    try { localStorage.setItem('check-lead-last', JSON.stringify(lead)) } catch {}
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-[#34c5c5]/10 via-[#F6F8FA] to-white">
        <style>{`@keyframes vspin{from{transform:rotate(0)}to{transform:rotate(360deg)}}@keyframes vpulse{0%,100%{transform:scale(1);opacity:.5}50%{transform:scale(1.5);opacity:0}}`}</style>

        {/* INTRO */}
        {phase === 'intro' && (
          <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
            <div className="text-center max-w-2xl mx-auto">
              <div className="inline-flex items-center gap-2 bg-[#34c5c5]/15 text-[#0D9488] rounded-full px-3 py-1 text-xs font-bold uppercase tracking-widest mb-5"><AudioLines className="w-3.5 h-3.5" /> Free · Resilience &amp; Readiness Check</div>
              <h1 className="text-4xl md:text-6xl font-black text-gray-900 leading-[1.04] mb-4">Are you mission-ready —<br className="hidden md:block" /> or running on fumes?</h1>
              <p className="text-lg md:text-xl text-gray-600 font-light mb-8">Built for <b className="text-gray-900">Veterans and first responders</b>. Speak for 30 seconds and our engine reads <b className="text-gray-900">10,000+ signals</b> in your voice to map your operational stress load, resilience, focus, and recovery capacity. Then we show you how to rebuild your edge.</p>
              ...
              <p className="text-xs text-gray-400 mt-4">No audio is uploaded or stored — analysis happens on your device. An illustrative readiness snapshot, not a medical or psychological screening.</p>
            </div>
            ...
            <div className="mt-12 bg-white rounded-2xl border border-gray-200 p-6">
              <p className="text-center text-xs font-bold uppercase tracking-wider text-gray-400 mb-4">Why readiness matters for those who serve</p>
              ...
              <p className="text-center text-[11px] text-gray-400 mt-5 max-w-2xl mx-auto">Inspired by the emerging vocal-biomarker field (Sonde Health, Ellipsis Health). Stat sources: SAMHSA, Ruderman Foundation, Nature.</p>
            </div>
          </section>
        )}
        ...
        {phase === 'recording' && (
          <section className="max-w-3xl mx-auto px-4 py-16 text-center min-h-[70vh] flex flex-col items-center justify-center">
            ...
            <h2 className="text-3xl md:text-4xl font-black text-gray-900 mb-2">Talk to me for 30 seconds.</h2>
            <p className="text-gray-500 mb-10">Your day, a mission, what is on your mind — there are no wrong words.</p>
            ...
            {micDenied && <p className="text-xs text-[#e07800] mt-3">Microphone unavailable — running a demo analysis so you can see your readiness board.</p>}
            <button onClick={stopRecording} className="mt-8 text-[#0D9488] font-bold text-sm border-2 border-[#34c5c5]/40 rounded-xl px-5 py-2.5 hover:bg-[#34c5c5]/5">I&rsquo;m done — analyze now</button>
          </section>
        )}
        ...
        {phase === 'processing' && (
          <section className="max-w-2xl mx-auto px-4 py-16 text-center min-h-[78vh] flex flex-col items-center justify-center">
            ...
            <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-2">Assessing your readiness signature</h2>
        ...
        {phase === 'gate' && (
          <section className="relative overflow-hidden min-h-[82vh] flex items-center justify-center bg-gradient-to-br from-[#33413f] via-[#0D5953] to-[#0D9488]">
            ...
            <form onSubmit={submitLead} className="relative z-10 w-full max-w-md bg-white/95 backdrop-blur rounded-3xl p-7 md:p-8 shadow-2xl m-4">
              ...
              <h2 className="text-2xl font-black text-gray-900 text-center mb-1">Your readiness board is ready 🔓</h2>
              <p className="text-gray-500 text-center text-sm mb-5">Where should we send your results?</p>
              <div className="space-y-2.5">
              ...
              <p className="text-[11px] text-gray-400 text-center mt-3">We respect your privacy. An illustrative readiness snapshot — not medical or psychological advice.</p>
            </form>
          </section>
        )}
        ...
        {phase === 'dashboard' && (
          <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div className="flex flex-wrap items-end justify-between gap-3 mb-6">
              <div>
                <p className="text-[#0D9488] font-bold uppercase tracking-[0.18em] text-xs mb-1">Resilience &amp; Readiness Check{name ? ` · ${name}` : ''}</p>
                <h1 className="text-3xl md:text-4xl font-black text-gray-900">Here&rsquo;s where you stand.</h1>
              </div>
            ...
            <div className="grid md:grid-cols-3 gap-5 mb-5">
              <div className="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col items-center justify-center">
                <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Overall Readiness</p>
                <Gauge value={p.overall} size={170} color="#0D9488" label="SCORE" />
              </div>
              <div className="grid grid-rows-2 gap-5">
                <div className="bg-white rounded-2xl border border-gray-200 p-5 flex items-center gap-4">
                  <Gauge value={p.stress} size={92} color="#e07800" />
                  <div><p className="font-black text-gray-900">Operational Stress Load</p><p className="text-sm text-gray-500">Nervous-system activation read from tone & pace.</p></div>
                </div>
                <div className="bg-white rounded-2xl border border-gray-200 p-5 flex items-center gap-4">
                  <Gauge value={p.recovery} size={92} color="#0D9488" />
                  <div><p className="font-black text-gray-900">Recovery Capacity</p><p className="text-sm text-gray-500">How well you reset between the demands.</p></div>
                </div>
              </div>
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-2 gap-3 mt-4 text-center">
                  <div className="bg-[#F6F8FA] rounded-xl p-3"><p className="text-2xl font-black text-[#0D9488]"><Counter to={p.dims.Drive} /></p><p className="text-[11px] text-gray-500 font-bold">Drive</p></div>
                  <div className="bg-[#F6F8FA] rounded-xl p-3"><p className="text-2xl font-black text-[#0D9488]"><Counter to={p.dims.Steadiness} /></p><p className="text-[11px] text-gray-500 font-bold">Steadiness</p></div>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-5 mb-5">
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Your 6-dimension readiness profile</p>
                <Radar dims={p.dims} />
              </div>
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
            ...
            <div className="bg-gradient-to-br from-[#0D9488] to-[#0a5d58] rounded-2xl p-6 md:p-8 text-white mb-5">
              ...
              <ul className="space-y-2 text-white/90 text-[15px]">
                <li>• Your strongest asset is <b>{topDim[0]}</b> ({topDim[1]}/100) — lead with it.</li>
                <li>• Your biggest opportunity is <b>{lowDim[0]}</b> ({lowDim[1]}/100); your <b>Operational Stress Load is {p.stress}</b> with a <b>Recovery Capacity of {p.recovery}</b>.</li>
                <li>• Readiness is rebuilt with a guided system — structured recovery, not white-knuckling it alone.</li>
              </ul>
            </div>

            {/* CTA */}
            <div className="bg-white rounded-2xl border-2 border-[#34c5c5]/40 p-6 md:p-8 text-center">
              <p className="text-[#0D9488] font-bold uppercase tracking-wider text-xs mb-2">Your next move</p>
              <h3 className="text-2xl md:text-3xl font-black text-gray-900 mb-3">Rebuild your edge with ShYft Master.</h3>
              <p className="text-gray-600 max-w-xl mx-auto mb-6">Your voice flagged where to focus. ShYft Master is the proprietary system that rebuilds resilience, focus, and recovery — guided, personalized, and proven for high-demand lives.</p>
              <a href="https://shyftmaster.com" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-gradient-to-r from-[#E8A849] to-[#e07800] text-white font-black text-lg px-9 py-4 rounded-2xl shadow-xl hover:shadow-2xl transition-shadow">
                Build My Readiness Plan <ArrowRight className="w-5 h-5" />
              </a>
              <div className="mt-5 pt-5 border-t border-gray-100">
                <p className="text-sm text-gray-600 mb-2">A Veteran? There&rsquo;s a mission built for you.</p>
                <a href="https://hernextmission.org" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[#0D9488] font-black hover:underline">Explore HER NEXT MISSION <ArrowRight className="w-4 h-4" /></a>
              </div>
              <div className="mt-4"><button onClick={() => { setPhase('intro'); setProfile(null) }} className="text-gray-400 text-sm font-semibold hover:text-[#0D9488]">Run another check</button></div>
            </div>

            <div className="mt-6 max-w-2xl mx-auto text-center">
              <div className="inline-flex items-center gap-2 text-sm font-bold text-[#e07800] bg-[#e07800]/10 rounded-full px-4 py-2">
                <Phone className="w-4 h-4" /> In crisis? Dial 988, then press 1 — the Veterans Crisis Line.
              </div>
              <p className="text-[11px] text-gray-400 mt-4">This resilience &amp; readiness snapshot is for informational and educational purposes only and is not medical, psychological, or diagnostic advice. If you or someone you serve with is struggling, please reach out to a qualified professional or the resources above.</p>
            </div>
          </section>
        )}
      </main>
